...
static CSemanodex* semOutbound = NULL;
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Nodex users should be well used to small numbers with many decimal places by now :)
...
void static ProcessOneShot()
{
    ...
    CAddress addr;
    CSemanodexGrant grant(*semOutbound, true);
    if (grant) {
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        CSemanodexGrant grant(*semOutbound);
        boost::this_thread::interruption_point();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemanodexGrant* grantOutbound, const char* pszDest, bool fOneShot)
{
    //
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Nodex Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (semOutbound == NULL) {
        // initialize semanodex
        int nMaxOutbound = min(MAX_OUTBOUND_CONNECTIONS, nMaxConnections);
        semOutbound = new CSemanodex(nMaxOutbound);
    }
