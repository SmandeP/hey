...
std::string HelpRequiringPassdxdase()
{
    return pwalletMain && pwalletMain->IsCrypted() ? "\nRequires wallet passdxdase to be set with walletpassdxdase call." : "";
}

void EnsureWalletIsUnlocked()
{
    if (pwalletMain->IsLocked() || pwalletMain->fWalletUnlockAnonymizeOnly)
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passdxdase with walletpassdxdase first.");
}
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            HelpExampleCli("walletpassdxdase", "\"mypassdxdase\" 30") +
...
Value walletpassdxdase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            "walletpassdxdase \"passdxdase\" timeout ( anonymizeonly )\n"
            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
            "This is needed prior to performing transactions related to private keys such as sending DXDs\n"
            "\nArguments:\n"
            "1. \"passdxdase\"     (string, required) The wallet passdxdase\n"
            "2. timeout            (numeric, required) The time to keep the decryption key in seconds.\n"
            "3. anonymizeonly      (boolean, optional, default=flase) If is true sending functions are disabled."
            "\nNote:\n"
            "Issuing the walletpassdxdase command while the wallet is already unlocked will set a new unlock\n"
            "time that overrides the old one. A timeout of \"0\" unlocks until the wallet is closed.\n"
            "\nExamples:\n"
            "\nUnlock the wallet for 60 seconds\n" +
            HelpExampleCli("walletpassdxdase", "\"my pass dxdase\" 60") +
            "\nUnlock the wallet for 60 seconds but allow Obfuscation only\n" + HelpExampleCli("walletpassdxdase", "\"my pass dxdase\" 60 true") +
            "\nLock the wallet again (before 60 seconds)\n" + HelpExampleCli("walletlock", "") +
            "\nAs json rpc call\n" + HelpExampleRpc("walletpassdxdase", "\"my pass dxdase\", 60"));

    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassdxdase was called.");

    // Note that the walletpassdxdase is stored in params[0] which is not mlock()ed
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSDXDASE_INCORRECT, "Error: The wallet passdxdase entered was incorrect.");
...
Value walletpassdxdasechange(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            "walletpassdxdasechange \"oldpassdxdase\" \"newpassdxdase\"\n"
            "\nChanges the wallet passdxdase from 'oldpassdxdase' to 'newpassdxdase'.\n"
            "\nArguments:\n"
            "1. \"oldpassdxdase\"      (string) The current passdxdase\n"
            "2. \"newpassdxdase\"      (string) The new passdxdase\n"
            "\nExamples:\n" +
            HelpExampleCli("walletpassdxdasechange", "\"old one\" \"new one\"") + HelpExampleRpc("walletpassdxdasechange", "\"old one\", \"new one\""));

    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassdxdasechange was called.");
    ...
    if (!pwalletMain->ChangeWalletPassdxdase(strOldWalletPass, strNewWalletPass))
        throw JSONRPCError(RPC_WALLET_PASSDXDASE_INCORRECT, "Error: The wallet passdxdase entered was incorrect.");
...
Value walletlock(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            "After calling this method, you will need to call walletpassdxdase again\n"
            "before being able to call any methods which require the wallet to be unlocked.\n"
            "\nExamples:\n"
            "\nSet the passdxdase for 2 minutes to perform a transaction\n" +
            HelpExampleCli("walletpassdxdase", "\"my pass dxdase\" 120") +
            "\nPerform a send (requires passdxdase set)\n" + HelpExampleCli("sendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 1.0") +
            "\nClear the passdxdase since we are done before 2 minutes is up\n" + HelpExampleCli("walletlock", "") +
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            "encryptwallet \"passdxdase\"\n"
            "\nEncrypts the wallet with 'passdxdase'. This is for first time encryption.\n"
            "After this, any calls that interact with private keys such as sending or signing \n"
            "will require the passdxdase to be set prior the making these calls.\n"
            "Use the walletpassdxdase call for this, and then walletlock call.\n"
            "If the wallet is already encrypted, use the walletpassdxdasechange call.\n"
            "Note that this will shutdown the server.\n"
            "\nArguments:\n"
            "1. \"passdxdase\"    (string) The pass dxdase to encrypt the wallet with. It must be at least 1 character, but should be long.\n"
            "\nExamples:\n"
            "\nEncrypt you wallet\n" +
            HelpExampleCli("encryptwallet", "\"my pass dxdase\"") +
            "\nNow set the passdxdase to use the wallet, such as for signing or sending DXDs\n" + HelpExampleCli("walletpassdxdase", "\"my pass dxdase\"") +
            "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"nodexaddress\" \"test message\"") +
            "\nNow lock the wallet again by removing the passdxdase\n" + HelpExampleCli("walletlock", "") +
    ...
    if (strWalletPass.length() < 1)
        throw runtime_error(
            "encryptwallet <passdxdase>\n"
            "Encrypts the wallet with <passdxdase>.");
...
Value reconsiderzerocoins(const Array& params, bool fHelp)
{
    ...
    if(pwalletMain->IsLocked())
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
                           "Error: Please enter the wallet passdxdase with walletpassdxdase first.");
