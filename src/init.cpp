...
// Copyright (c) 2017 The Nodex developers
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DXD and zDXD money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DXD/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DXD/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DXD/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzdxd=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDxd minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizenodexamount=<n>     " + strprintf(_("Keep N DXD anonymized (default: %u)"), 0) + "\n";
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZDXDMinted();
                        RecalculateZDXDSpent();
                    }
                    RecalculateDXDSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZDxdBackups = GetBoolArg("-backupzdxd", true);
        pwalletMain->setZDxdAutoBackups(fEnableZDxdBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeNodexAmount < 2) nAnonymizeNodexAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1DXD+1000 == (.1DXD+100)*10
       10DXD+10000 == (1DXD+1000)*10
    */
