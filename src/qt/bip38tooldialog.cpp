...
bool isValidPassdxdase(QString strPassdxdase, QString& strInvalid)
{
    for (int i = 0; i < strPassdxdase.size(); i++) {
        if (!validChar.contains(strPassdxdase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPassdxdase[i]))
                continue;

            strInvalid = strPassdxdase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPassdxdase = ui->passdxdaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassdxdase(qstrPassdxdase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passdxdase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qstrPassdxdase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPassdxdase = ui->passdxdaseIn_DEC->text().toStdString();
    string strKey = ui->encryptedKeyIn_DEC->text().toStdString();

    uint256 privKey;
    bool fCompressed;
    if (!BIP38_Decrypt(strPassdxdase, strKey, privKey, fCompressed)) {
        ui->statusLabel_DEC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passdxdase and try again."));
        return;
    }
