...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DXD.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passdxdase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passdxdase);
    } else {
...
bool WalletModel::setWalletLocked(bool locked, const SecureString& passDxdase, bool anonymizeOnly)
{
    if (locked) {
        // Lock
        ...
        // Unlock
        return wallet->Unlock(passDxdase, anonymizeOnly);
    }
}
...
bool WalletModel::changePassdxdase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassdxdase(oldPass, newPass);
    }
    return retval;
}
